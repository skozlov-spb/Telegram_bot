...
async def process_expert_selection(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "admin_back_to_panel":
        ...
        if not experts:
            await callback.message.answer("⚠️*Эксперты отсутствуют.*",reply_markup=main_kb(user_id), parse_mode="Markdown")
            await callback.answer()
            await db_utils.db.close()
            return
...
async def handle_broadcast_confirmation(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer(f"⚠️Произошла ошибка при рассылке: {str(e)}", reply_markup=admin_panel_kb())
